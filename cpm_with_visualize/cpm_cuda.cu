...
int main(int argc, char* argv[]) {
    ...
    for (int mcs = 0; mcs < NUM_MCS; mcs++) {
        ...
        if (flagDense){
            ...
            //solve_linear_system_cuSOLVER(chemical, E_field_x, E_field_y, LATTICE_SIZE);
        } else {
            solve_sparse_linear_system(chemical, E_field_x, E_field_y, LATTICE_SIZE); // Fa uso di SuiteSparse con le matrici sparse
            //solve_sparse_linear_system_cuda(chemical, E_field_x, E_field_y, LATTICE_SIZE);
